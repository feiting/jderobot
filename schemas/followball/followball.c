...
#define POS_MAX_TILT 120.0
  ...
  if (data_filter.pixeles > MIN_PIXELES) {
    ...
    if (data_filter.distancia > MIN_DISTANCIA) {   /* banda muerta */

      /* Calculamos la distancia que se tiene que mover la PAN-TILT */

      angulo_x = ENCOD_TO_DEG * ((int) abs(data_filter.x) );   //  CTE_PTU);
      angulo_y = ENCOD_TO_DEG * ((int) abs(data_filter.y) );   //  CTE_PTU);
...
void followball_guiresume(void)
{
  ...
  fl_set_slider_value(fd_followballgui->Imax,i_max);
  fl_set_slider_bounds(fd_followballgui->Hmin,2.*PI,0);
  fl_set_slider_value(fd_followballgui->Hmin,h_min);
  fl_set_slider_bounds(fd_followballgui->Hmax,2.*PI,0);
